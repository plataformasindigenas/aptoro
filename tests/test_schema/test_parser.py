...
class TestParseTypeString:
    ...
    def test_str_with_constraints(self):
    ...
    def test_list_with_default_empty(self):
        ft = parse_type_string("list[str] = []")
        assert ft.base == BaseType.LIST
        assert ft.has_default
        assert ft.default == []
...
class TestLoadSchema:
    ...
    def test_load_simple_schema(self, sample_schema_path: Path):
    ...
    def test_schema_has_fields(self, sample_schema_path: Path):
        schema = load_schema(sample_schema_path)
    ...
    def test_schema_field_types(self, sample_schema_path: Path):
        schema = load_schema(sample_schema_path)
    ...
    def test_schema_with_constraints(self, sample_schema_path: Path):
        schema = load_schema(sample_schema_path)
        pos = schema.get_field("pos")
        assert pos is not None
        assert pos.field_type.constraints == ("noun", "verb", "adj", "adv")

    def test_schema_inheritance(self, child_schema_path: Path):
    ...
    def test_schema_not_found(self, fixtures_dir: Path):
...
class TestSchemaProperties:
    ...
    def test_required_fields(self, sample_schema_path: Path):
        schema = load_schema(sample_schema_path)
    ...
    def test_optional_fields(self, sample_schema_path: Path):
        schema = load_schema(sample_schema_path)
    ...
    def test_field_names(self, sample_schema_path: Path):
        schema = load_schema(sample_schema_path)
